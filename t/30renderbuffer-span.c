...
int main(int argc, char *argv[])
{
  ...
  // Absolute spans
  {
    ...
    tickit_renderbuffer_text_at(rb, 0, 1, "text span", fg_pen);
  ...
  // UTF-8 handling
  {
    int cols = tickit_renderbuffer_text_at(rb, 6, 0, "somé text ĉi tie", NULL);
  ...
  // VC spans
  {
    ...
    tickit_renderbuffer_goto(rb, 0, 2); tickit_renderbuffer_text(rb, "text span", fg_pen);
    tickit_renderbuffer_goto(rb, 1, 2); tickit_renderbuffer_erase(rb, 5, fg_pen);
  ...
  // Translation
  {
    ...
    tickit_renderbuffer_text_at(rb, 0, 0, "at 0,0", NULL);
